...
def getAuthors(pdfXML, pdfNS):
	...
	author = pdfXML.find('./pdf:head/pdf:meta[@name=\'Author\']', pdfNS).attrib['content']
	if author is None:
		author = "Not done yet."
	return author
...
# filename = sys.argv[1]
filename = "/home/wink/fac/projet/Corpus_2021/Iria_Juan-Manuel_Gerardo.pdf"
pdfProcess = subprocess.Popen("/usr/bin/pdftotext -bbox-layout {} -".format(filename), stdout=subprocess.PIPE, shell=True)
